...
/* Private Functions */
static u32 clock_setup(USART_BaudRate baud, u32 sclk_khz, USART_OverSample over8);
...
// MOVE TO a u8 pointer can slim down the data being transfered here. Set up test for reading USART and reflecting!!
int usart_read(USART_TypeDef *ptr, u8* buf, int len){
    ...
    while(i < len) {
        if (usart_get_read(ptr)) {
            buf[i] = get_ptr_vol_raw_u8((volatile u8 *)&ptr->RDR);
            t = 0;
            i++;
        }
...
void usart_write(USART_TypeDef *ptr, u8* buf, int len){
    ...
    while(i < len){
        if (get_ptr_vol_bit_u32(&ptr->ISR, TXE_BIT)) {
            set_ptr_vol_raw_u8((volatile u8 *)&ptr->TDR, buf[i]);
            i++;
        }
    }
...
static u32 clock_setup(USART_BaudRate baud, u32 sclk_khz, USART_OverSample over8) {
    if (over8 == USART_Oversample_8) {
        u32 baud_div = ((sclk_khz * SCLK_HZ) * 2) / baud;
