...
/* Read Function */
u32 spi_read(SPI_TypeDef *ptr, u8* buf, int len) {
    u32 i = 0; // Index based on len
    u32 t = 0; // Time Out Rotation
    
    while(i < len) {
        if (get_ptr_vol_u32(&ptr->SR, FRLVL_OFFSET, LVL_MASK) != 0) {
            buf[i] = get_ptr_vol_raw_u8((volatile u8 *)&ptr->DR);
            t = 0;
            i++;
        }
...
/* Write Function */
void spi_write(SPI_TypeDef *ptr, u8* buf, int len) {
    u32 i = 0;
    
    while(i < len) {
        if (get_ptr_vol_u32(&ptr->SR, FTLVL_OFFSET, LVL_MASK) != 3) {
            set_ptr_vol_raw_u8((volatile u8 *)&ptr->DR, buf[i]);
            i++;
        }
    }
...
u32 spi_error_byte(SPI_TypeDef *ptr) {
