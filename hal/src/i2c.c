...
void i2c_stop_bus(I2C_TypeDef *ptr) { 
    ...
    volatile u32 i = 0; // CONVERT TO FAULT TIMER, VOLITILE WILL PREVENT OPTIMIZATION
    while (i < 100000) {
        i++;
    }
}
...
bool i2c_start(I2C_TypeDef *ptr) {
    ...
    u32 i = 0; // CONVERT TO FAULT TIMER
...
bool i2c_read(I2C_TypeDef *ptr, u8* buf, int len) {
    ...
    while(i < len){
        u32 t = 0; // CONVERT TO FAULT TIMER
        ...
        buf[i] = get_ptr_vol_raw_u8((volatile u8*)&ptr->RXDR);
        i++;
    }
    return true;
}
...
u8 i2c_read_u8(I2C_TypeDef *ptr) {
    ...
    return get_ptr_vol_raw_u8((volatile u8 *)&ptr->RXDR);
}
